...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        sample_mean = np.mean(x, axis=0, keepdims=True) # (D,)
        sample_var = np.mean((x - sample_mean)**2, axis=0, keepdims=True) # (D,)
...
def batchnorm_backward_alt(dout, cache):
    ...
    dbeta = np.sum(dout, axis=0)
    N = x.shape[0]
    dx = (dout / np.sqrt(sample_var + eps) - 
      2 * (x - sample_mean) * dout / (N * (sample_var + eps) ** 1.5) -
      np.sum(dout / N, axis=0))
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    sample_var = np.mean((x - sample_mean)**2, axis=1, keepdims=True) # (N, 1)

    sd = np.sqrt(sample_var + eps) #(N, 1)
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    out = np.zeros((N, F, H_prime, W_prime), dtype=float)
    for n in range(N):
      for height in range(H_prime):
        for width in range(W_prime):
          for filter_num in range(F):
            out[n,filter_num,height,width] = np.sum(w[filter_num,:,:,:] * padded_x[n,:,stride*height:stride*height+HH,stride*width:stride*width+WW]) + b[filter_num]
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def max_pool_forward_naive(x, pool_param):
    ...
    W_prime = 1 + int((H - pool_width) / stride)
    
    out = np.zeros((N, C, H_prime, W_prime), dtype=float)
    for n in range(N):
      for height in range(H_prime):
        for width in range(W_prime):
          out[n,:,height,width] = np.max(x[n,:,height*stride:height*stride+pool_height,width*stride:width*stride+pool_width], axis=(1, 2))
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def max_pool_backward_naive(dout, cache):
    ...
    for n in range(N):
      for c in range(C):
        for height in range(H_prime):
          for width in range(W_prime):
            max_index_flat = np.argmax(x[n,c,height*stride:height*stride+pool_height,width*stride:width*stride+pool_width])
            max_index = np.unravel_index(max_index_flat, (pool_height, pool_width))
            dx[n, c, height*stride+max_index[0], width*stride+max_index[1]] += dout[n, c, height, width]
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    N, C, H, W = x.shape
    x.reshape(N, C // G, G, H, W)
    out, cache = layernorm_forward(x.reshape(N, C // G, G, H, W), gamma.reshape(1, C // G, G, 1, 1), beta.reshape(1, C // G, G, 1, 1), gn_param)
...
def spatial_groupnorm_backward(dout, cache):
    ...
    dout = dout.reshape(N, C // G, G, H, W)
    x_hat, gamma, x, sample_mean, sample_var, eps, sd = cache

    dgamma = np.sum(dout * x_hat, axis=(0, 3, 4), keepdims=True)
    dbeta = np.sum(dout, axis=(0, 3, 4), keepdims=True)
