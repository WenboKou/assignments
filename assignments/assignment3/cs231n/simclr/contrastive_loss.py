...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        for i in range(2*N):
          if i == k:
            continue
          total += torch.exp(sim(z_k, out[i])/tau)
        total_loss -= torch.log(torch.exp(sim(z_k, z_k_N)/tau) / total)

        total = 0
        for i in range(2*N):
          if i == k+N:
            continue
          total += torch.exp(sim(z_k_N, out[i])/tau)
        total_loss -= torch.log(torch.exp(sim(z_k_N, z_k)/tau) / total)
...
def sim_positive_pairs(out_left, out_right):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    pos_pairs = torch.diag(out_left @ out_right.T) / (torch.linalg.norm(out_left, dim=1) * torch.linalg.norm(out_right, dim=1))
...
def compute_sim_matrix(out):
    ...
    sim_matrix = (out @ out.T) / (torch.linalg.norm(out, dim=1).reshape(-1, 1) @ torch.linalg.norm(out, dim=1).reshape(1, -1))
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    sim_pos_pairs = sim_positive_pairs(out, torch.cat([out_right, out_left], dim=0)) # [2*N, 1]
    ...
    numerator = torch.exp(sim_pos_pairs / tau) # [2*N, 1]
